...
const TherapistMessaging = () => {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (selectedFile) {
        ...
        formData.append('receiverUsername', selectedAppointment.patientUsername);
  ...
  const filteredAppointments = appointments.filter(appt => 
    (appt.patientFullName || appt.patientUsername).toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  const downloadTranscript = async () => {
    ...
    try {
      const response = await api.get(`/transcript/${selectedAppointment._id}`);
      const blob = new Blob([response.transcript], { type: 'text/plain' });
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      // Use full names in filename
      const filename = response.patientFullName 
        ? `${response.patientFullName.replace(/[^a-zA-Z0-9\s]/g, '').replace(/\s+/g, '_')}_transcript.txt`
        : `${selectedAppointment.patientUsername}_transcript.txt`;
      a.download = filename;
  ...
  return (
    <div className="flex min-h-screen bg-[#EBEDE9] h-screen overflow-x-hidden">
      <TherapistSidebar current="messages" />
      ...
      <div className="flex-1 lg:ml-64 p-4 sm:p-6">
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 sm:mb-6 space-y-4 sm:space-y-0">
          <div>
            ...
            <p className="text-sm sm:text-base text-gray-600 mt-1">Chat with your patients and manage your conversations</p>
          </div>
        ...
        <div className="flex-1 flex flex-col lg:flex-row h-full min-h-0">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between">
                            <h3 className="font-semibold text-gray-900 truncate text-sm sm:text-base">
                              {appt.patientFullName || appt.patientUsername}
                            </h3>
          ...
          {/* Chat Window */}
          <div className="flex-1 flex flex-col bg-white h-full lg:border-l lg:border-gray-200">
            {selectedAppointment ? (
              <>
                {/* Chat Header */}
                            ...
                            {msg.attachment && msg.attachment.originalName && (
                              <div className={`mt-2 sm:mt-3 p-2 sm:p-3 rounded-lg ${
                                ...
                                <div className="flex items-center space-x-1.5 sm:space-x-2">
                                  <FaFile className="flex-shrink-0 text-sm sm:text-base" />
                                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mb-2 sm:mb-3 p-2 sm:p-3 bg-blue-50 rounded-lg border border-blue-200">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-1.5 sm:space-x-2">
                          ...
                          <span className="text-xs sm:text-sm text-blue-800">{selectedFile.name}</span>
              ...
              <div className="flex-1 flex items-center justify-center">
                <div className="text-center">
                  <FaComments className="text-4xl sm:text-6xl text-gray-300 mx-auto mb-3 sm:mb-4" />
                  <h3 className="text-lg sm:text-xl font-semibold text-gray-600 mb-2">Select a conversation</h3>
                  <p className="text-sm sm:text-base text-gray-500">Choose a patient from the list to start messaging</p>
                </div>
              </div>
            )}
          </div>
...
export default TherapistMessaging;
